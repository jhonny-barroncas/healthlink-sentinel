...
- O painel Starlink agora explicita a associação operacional do agente: cada telemetria é exibida dentro da unidade cadastrada, com cidade/UF legíveis, além das coordenadas retornadas pela antena e do percentual real de obstrução quando disponível.
- O detalhe do equipamento exibe a última coleta em tabela horizontal, com cada métrica em sua própria coluna, coordenadas da antena separadas da localidade cadastrada da unidade e origem `local_agent`; quando não há amostra, informa que está aguardando o agente.
- O adapter Starlink agora aceita variações de envelope/campo da resposta gRPC de localização (`get_location`, `dish_get_location`, `lla`, `LLA`, latitude/longitude) e registra no console se a antena retornou coordenadas válidas ou `N/D`. O diagnóstico `agent:starlink:check` também exibe a localização.
...
- O agente local agora envia os alertas individuais retornados pela antena, além da métrica agregada `alertsActive`. A API reconcilia esses alertas por equipamento: cria incidentes em `alerts`, registra abertura/recuperação em `alert_events` e `monitoring_events`, mantém incidentes ativos sem duplicá-los e encerra os que deixam de aparecer na coleta.
- A perda de heartbeat do agente também é um incidente próprio, independente dos alertas internos da antena. O watchdog da API verifica agentes ativos a cada 15 segundos e, após 30 segundos sem comunicação, abre o incidente crítico `Agente sem comunicação`; o próximo heartbeat ou lote de telemetria registra a recuperação. Os heartbeats contínuos não são persistidos como eventos.
- O bundle do agente com esse contrato foi promovido para a versão `1.0.3`; uma resposta da antena sem o campo de alertas não encerra incidentes existentes por engano.
